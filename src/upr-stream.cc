...
PassiveStream::PassiveStream(EventuallyPersistentEngine* e, UprConsumer* c,
    ...
    LOG(EXTENSION_LOG_WARNING, "%s Attempting to add %s stream for vbucket %d "
        "with start seqno %llu, end seqno %llu, vbucket uuid %llu and high "
        "seqno %llu", consumer->logHeader(), type, vb, st_seqno, en_seqno,
        vb_uuid, hi_seqno);
}
